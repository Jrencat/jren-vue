...
import { downloadPdfByUrl, downloadPdfByBase64 } from '@/api/downloadUtils';
...
export default {
  ...
  methods: {
    ...
    operationItem(row) {
      let msg = '';
      if (row.type === 'urlDownload') {
        msg = this.$_checkIsEmpty(this.dataForm.url) ? '请输入URL' : '';
      }
      if (row.type === 'base64Download') {
      ...
      this.dataProcessimg(row);
    },
    async dataProcessimg(row) {
      let url = '';
      if (row.type === 'urlDownload') {
        url = this.dataForm.url;
        downloadPdfByUrl(this, url, '文件.pdf');
      }
      if (row.type === 'base64Download') {
        downloadPdfByBase64(this.dataForm.base64, '文件.pdf');
      }
    }
  }
}
